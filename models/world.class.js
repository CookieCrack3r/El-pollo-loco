...
class World {
    ...
    checkEndbossArea() {
        if (this.character.x >= 2200 && !this.level.endboss[0].isEntering) {
    ...
    checkCollisionsWithCoins() {
        this.level.coins.forEach((coin, index) => {
            if (this.character.isColliding(coin) && this.collectedCoins < 5) {
                this.collectedCoins++;
    ...
    draw() {
        ...
        if (this.level.endboss[0] && (this.level.endboss[0].isEntering || this.level.endboss[0].x < 3000)) {
